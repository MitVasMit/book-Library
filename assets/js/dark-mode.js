...
(function() {
    ...
    function initDarkMode() {
        ...
        const darkMode = localStorage.getItem('darkMode') || sessionStorage.getItem('darkMode');
    ...
    class DarkModeManager {
        ...
        retryBinding() {
            ...
            const attemptBinding = () => {
                ...
                const availableToggles = allToggles.filter(id => document.getElementById(id));
                
                if (availableToggles.length < allToggles.length) {
                    retryCount++;
                    console.log(`Retry ${retryCount}: Some toggles not found, retrying in 200ms...`);
    ...
    if (!window.darkModeManager) {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                if (!window.darkModeManager) {
                    console.log('DOM ready, initializing DarkModeManager...');
            ...
            if (!window.darkModeManager) {
                console.log('Page already loaded, initializing DarkModeManager...');
                window.darkModeManager = new DarkModeManager();
            }
        }
    } else {
        console.log('DarkModeManager already exists, reusing...');
    }
    ...
    // Add a test function to debug dark mode
    window.testDarkMode = function() {
        console.log('=== Dark Mode Debug ===');
        console.log('DarkModeManager exists:', !!window.darkModeManager);
        console.log('localStorage darkMode:', localStorage.getItem('darkMode'));
        console.log('sessionStorage darkMode:', sessionStorage.getItem('darkMode'));
        console.log('HTML dark class:', document.documentElement.classList.contains('dark'));
        console.log('HTML data-theme:', document.documentElement.getAttribute('data-theme'));
        
        // Check if toggle buttons exist
        const toggles = ['darkToggle', 'darkToggleDesktop', 'darkToggleTablet', 'darkToggleStandalone'];
        toggles.forEach(id => {
            const toggle = document.getElementById(id);
            console.log(`${id}:`, !!toggle, toggle ? toggle.textContent : 'N/A');
        });
        
        // Test toggle functionality
        const testToggle = document.getElementById('darkToggle');
        if (testToggle) {
            console.log('Test toggle clickable:', testToggle.onclick !== null);
        }
    };
    ...
    window.addEventListener('load', () => {
        console.log('Page load complete, final dark mode check...');
        ...
        console.log('Dark mode state:', { isDark, theme, localStorage: localStorage.getItem('darkMode') });
    });
